...
public class MainFrame extends WebFrame {
        ...
        addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent we) {
                try {
                    ...
                    if (!close.canceled()) System.exit(0);
    ...
    public boolean checkOrphanGroupingStateIfNeeded() {
        if (state.guiConf.lastTab == 0) {
            return contextView.checkOrphanGroupingState();
        }
        return true;
    }
    ...
    public void showLatticeEditor() {
        // (F1) : bloquer si l'user est en cours de groupage avec des orphelins
    ...
    private class SwitchTab extends AbstractAction {
        private int tabnr; private WebTabbedPane tabPane;
        public SwitchTab(int i, WebTabbedPane tabPane) { tabnr = i; this.tabPane = tabPane; }
        public void actionPerformed(ActionEvent arg0) { tabPane.setSelectedIndex(tabnr); }
    }

    @SuppressWarnings("serial")
    public class StillCalculatingDialog extends WebDialog {
        private boolean yes; private boolean no;
        public StillCalculatingDialog() {
            super(MainFrame.this, LocaleHandler.getString("MainFrame.StillCalculatingDialog.StillCalculatingDialog.title"), true);
            final WebOptionPane pane = new WebOptionPane(LocaleHandler.getString("MainFrame.StillCalculatingDialog.StillCalculatingDialog.pane"));
            Object[] options = {
            ...
            pane.setOptions(options); pane.setMessageType(WebOptionPane.INFORMATION_MESSAGE);
            pane.addPropertyChangeListener(new PropertyChangeListener() {
                public void propertyChange(PropertyChangeEvent e) {
                    if (isVisible() && e.getSource() == pane && e.getPropertyName().equals(WebOptionPane.VALUE_PROPERTY)) setVisible(false);
                }
            });
            setModal(true); setContentPane(pane); pack();
            setDefaultCloseOperation(WebDialog.DO_NOTHING_ON_CLOSE);
            Util.centerDialogInsideMainFrame(MainFrame.this, this); setVisible(true);
            String n = (String) pane.getValue();
            if (n.equals(LocaleHandler.getString("MainFrame.StillCalculatingDialog.StillCalculatingDialog.options.wait"))) { yes = true; no = false; }
            else { no = true; yes = false; }
        }
        public boolean isYes() { return yes; } public boolean isNo() { return no; }
    }
    ...
    public class OverwritingFileDialog extends WebDialog {
        ...
        public OverwritingFileDialog(File file) {
            ...
            optionPane.setOptions(options); setContentPane(optionPane);
            optionPane.addPropertyChangeListener(new PropertyChangeListener() {
                public void propertyChange(PropertyChangeEvent e) {
                    if (isVisible() && e.getSource() == optionPane && e.getPropertyName().equals(WebOptionPane.VALUE_PROPERTY)) setVisible(false);
                }
            });
            pack(); setDefaultCloseOperation(WebDialog.DO_NOTHING_ON_CLOSE);
            Util.centerDialogInsideMainFrame(MainFrame.this, this); setVisible(true);
            String n = (String) optionPane.getValue();
            if (n.equals(LocaleHandler.getString("yes"))) { yes = true; no = false; }
            else if (n.equals(LocaleHandler.getString("no"))) { no = true; yes = false; }
        }
        public boolean isYes() { return yes; } public boolean isNo() { return no; }
    }
    ...
    public class UnsavedChangesDialog extends WebDialog {
        private boolean cancel; private boolean yes; private boolean no;
        public UnsavedChangesDialog() {
            super(MainFrame.this, LocaleHandler.getString("MainFrame.UnsavedChangesDialog.UnsavedChangesDialog.title"), true);
            Object[] options = { LocaleHandler.getString("yes"), LocaleHandler.getString("no"), LocaleHandler.getString("cancel") };
            final WebOptionPane optionPane = new WebOptionPane(
            ...
            pack(); Util.centerDialogInsideMainFrame(MainFrame.this, this); setVisible(true);
            String n = (String) optionPane.getValue();
            if (n.equals(LocaleHandler.getString("yes")))         { yes = true; no = false; cancel = false; }
            else if (n.equals(LocaleHandler.getString("cancel"))) { cancel = true; no = false; yes = false; }
